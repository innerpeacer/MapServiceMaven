...
class TYBrtMvtEncoder_v1 implements TYIBrtMvtEncoder {
	...
	public VectorTile.Tile encodeBrtTile(TYBrtGeometrySet geomSet,
		...
		for (int i = 0; i < TYBrtVectorTileParams.LAYER_LIST_v1.length; ++i) {
			String layerName = TYBrtVectorTileParams.LAYER_LIST_v1[i];
			...
			final MvtTileGeomResult bufferedTileGeom = TYBrtJtsAdapter
					.createTileGeom(filteredGeomList, tileEnvelope,
							clipEnvelope, geomFactory,
							TYBrtVectorTileParams.DEFAULT_MVT_PARAMS,
							TYBrtVectorTileParams.ACCEPT_ALL_FILTER, tile);
			final List<VectorTile.Tile.Feature> features = TYBrtJtsAdapter
