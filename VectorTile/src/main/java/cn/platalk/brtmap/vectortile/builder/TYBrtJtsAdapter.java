...
final class TYBrtJtsAdapter {
	...
	public static MvtTileGeomResult createTileGeom(List<Geometry> g,
			...
			MvtIGeometryFilter filter, TYBrtTileCoord tile) {
		...
		for (Geometry nextInterGeom : intersectedGeoms) {
			...
			nextTransformGeom = TYBrtGeomTransform.brtTransform(nextInterGeom,
					tile, mvtLayerParams);
	...
	private static VectorTile.Tile.Feature toFeature(Geometry geom,
		...
		final VectorTile.Tile.GeomType mvtGeomType = TYBrtJtsAdapter
				.toGeomType(geom);
