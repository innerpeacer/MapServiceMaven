...
public class TYBrtGeomTransform {
	static GeometryFactory factory = new GeometryFactory();

	public static Geometry brtTransform(Geometry g, TYBrtTileCoord tile,
	...
	private static Point transformedPoint(Point p, TYBrtTileCoord tile,
			MvtLayerParams layerParams) {
		return createTransformedPoint(p, tile, layerParams);
	}

	private static MultiPoint transformedMultiPoint(MultiPoint mp,
			TYBrtTileCoord tile, MvtLayerParams layerParams) {
	...
	private static MultiLineString transformedMultiLineString(
			MultiLineString mls, TYBrtTileCoord tile, MvtLayerParams layerParams) {
		GeometryFactory factory = new GeometryFactory();
	...
	private static Polygon transformedPolygon(Polygon p, TYBrtTileCoord tile,
			MvtLayerParams layerParams) {
	...
	private static Point createTransformedPoint(Point p, TYBrtTileCoord tile,
			MvtLayerParams layerParams) {
	...
	private static Coordinate createTransformedCoordinate(Coordinate c,
			TYBrtTileCoord tile, MvtLayerParams layerParams) {
		double xTile = TYBrtTileCoord.lon2Tile(c.x, tile.zoom);
		double yTile = TYBrtTileCoord.lat2Tile(c.y, tile.zoom);
