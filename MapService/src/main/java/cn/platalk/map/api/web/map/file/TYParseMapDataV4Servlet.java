...
import cn.platalk.core.map.shp.TYShpGeneratingTaskV4;
import cn.platalk.core.map.shp.TYShpGeneratingTaskV4.TYShpGeneratingTaskListenerV4;
...
@WebServlet("/web/ParseMapDataV4")
public class TYParseMapDataV4Servlet extends TYBaseHttpServlet {
	...
	protected void doGet(HttpServletRequest request, final HttpServletResponse response)
		...
		TYShpGeneratingTaskV4 task = new TYShpGeneratingTaskV4(TYMapEnvironment.GetShpRootDir(buildingID), buildingID);
		task.addTaskListener(new TYShpGeneratingTaskListenerV4() {
			...
			public void didFinishGeneratingTask(TYShpGeneratingTaskV4 task) {
				...
				List<TYIconTextSymbolRecord> iconTextSymbolRecords = task.getIconTextSymbols();
				System.out.println(iconTextSymbolRecords.size() + " iconTextSymbols");
...

				{
					...
					symbolDB.insertIconTextSymbolRecords(task.getIconTextSymbols(), buildingID);
				...
				try {
					...
					resBuffer.append(iconTextSymbolRecords.size()).append(" iconTextSymbols").append("\n");
