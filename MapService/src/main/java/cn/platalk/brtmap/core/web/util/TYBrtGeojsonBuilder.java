package cn.platalk.brtmap.core.web.util;
...
public class TYBrtGeojsonBuilder {
	...
	private static JSONArray buildGeojsonPointFeature(Point point) throws JSONException {
		...
		double xy[] = TYBrtCoordProjection.mercatorToLonLat(point.getX(), point.getY());
		coordArray.put(xy[0]);
		coordArray.put(xy[1]);
		return coordArray;
	}
	...
	private static JSONArray buildGeojsonLineString(LineString ls) throws JSONException {
		...
		for (int i = 0; i < pointCount; ++i) {
			...
			double xy[] = TYBrtCoordProjection.mercatorToLonLat(coord.x, coord.y);
