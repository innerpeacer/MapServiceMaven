...
import cn.platalk.map.entity.base.impl.map.TYMapSize;
...
class IPSqliteBuildingParams {
    ...
    public static List<TYBuilding> BuildingListFromRecords(List<IPSqlRecord> records) {
        ...
        for (IPSqlRecord record : records) {
            ...
            String wgs84PointString = record.getString(FIELD_BUILDING_17_WGS84_CALIBRATION_POINT);
            if (wgs84PointString != null && wgs84PointString.length() > 0) {
                building.setWgs84CalibrationPoint(stringToDoubleArray(wgs84PointString));
            }

            String wtPointString = record.getString(FIELD_BUILDING_18_WT_CALIBRATION_POINT);
            if (wtPointString != null && wtPointString.length() > 0) {
                building.setWtCalibrationPoint(stringToDoubleArray(wtPointString));
            }

            String dataVersion = record.getString(FIELD_BUILDING_19_DATA_VERSION);
            if (dataVersion != null && dataVersion.length() > 0) {
                building.setDataVersion(dataVersion);
            }

            String centerString = record.getString(FIELD_BUILDING_20_CENTER);
